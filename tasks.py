...
crime = np.load("./data/crime_counts.npy")
popus = np.load("./data/popus_count.npy")
cd = json.load(open("./data/mh_cd.json"))
cd_labels = np.zeros(270)
for i in range(270):
...
def predict_crime(emb):
    ...
    mae, rmse = compute_metrics(y_pred, y_test)
    print('==========================================>')
...
def lu_classify(emb):
    ...
    np.save('./data/predict_label.npy', emb_labels)
    print('==========================================>')
    print("Land Usage Clustering")
